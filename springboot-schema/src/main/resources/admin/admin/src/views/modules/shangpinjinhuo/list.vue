  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
          ...
          <el-button
            v-if="isAuth('shangpinjinhuo','新增')"
            type="primary"
            @click="addOrUpdateHandler()"
            round
          >新增</el-button>
          <el-button
            v-if="isAuth('shangpinjinhuo','删除')"
            ...
            <el-table-column
                ...
                width="50">
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.ispay!='已支付' && isAuth('shangpinjinhuo','支付') " type="text" icon="el-icon-edit" size="small" @click="payHandler(scope.row)">支付</el-button>
                  </template>
                </el-table-column>
                                                      <el-table-column
                  prop="shhf"
              ...
              <el-table-column
                ...
                label="操作">
                <template slot-scope="scope">
                                <el-button v-if="isAuth('shangpinjinhuo','查看')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                                                <el-button v-if="isAuth('shangpinjinhuo','修改')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                <el-button v-if="isAuth('shangpinjinhuo','删除')" type="text" icon="el-icon-delete" size="small" @click="deleteHandler(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
                                                                                                                                                                                                                                                                                                <el-pagination
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
      ...
      width="800">  
                                                                                                          <div id="jinhuoshuliangChart" style="width:100%;height:600px;"></div>
                                                                                                                                                                <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
            payHandler(row){
      this.$storage.set('paytable','shangpinjinhuo');
      ...
      this.$nextTick(()=>{
                                                                                  // mingcheng mingcheng
                                                                                                                    //  jinhuoshuliang
                                                                                                                                                                                                                                                                                                   // xcolumn ycolumn mingcheng jinhuoshuliang
                var jinhuoshuliangChart = this.$echarts.init(document.getElementById("jinhuoshuliangChart"),'macarons');
        this.$http({
            url: `value/shangpinjinhuo/mingcheng/jinhuoshuliang`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                    ...
                    var option = {};
                        ...
                        series: [{
                            ...
                            type: 'bar'
                                        ...
                                        // 使用刚指定的配置项和数据显示图表。
                    jinhuoshuliangChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        jinhuoshuliangChart.resize();
                    };
                }
            }
        });
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
                    ...
                    tupian: row.tupian,
                    jinjia: row.jinjia,
                    jinhuoshuliang: row.jinhuoshuliang,
                    zongjiage: row.zongjiage,
                    gongyingshang: row.gongyingshang,
                    dingdanriqi: row.dingdanriqi,
                    beizhu: row.beizhu,
